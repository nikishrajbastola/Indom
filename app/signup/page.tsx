    ...
    <main className={auth.page}>
      <section className={auth.card}>
        ...
        <form className={auth.form} onSubmit={handleSignup}>
          <FormField label="Full name" htmlFor="full-name"><TextInput id="full-name" name="fullName" value={fullName} onChange={(event) => { setFullName(event.target.value); setFormError(""); }} autoComplete="name" required /></FormField>
          ...
          <FormField label="Password" htmlFor="password">
            <div className={auth.passwordField}><TextInput id="password" name="password" type={showPassword ? "text" : "password"} value={password} onChange={(event) => { setPassword(event.target.value); setFormError(""); }} autoComplete="new-password" aria-describedby="password-requirements" aria-invalid={password.length > 0 && !passwordValid} required /><button type="button" className={auth.passwordToggle} onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? "Hide password" : "Show password"} aria-pressed={showPassword}>{showPassword ? "Hide" : "Show"}</button></div>
          </FormField>
          ...
          <FormField label="Confirm password" htmlFor="confirm-password">
            <div className={auth.passwordField}><TextInput id="confirm-password" name="confirmPassword" type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(event) => { setConfirmPassword(event.target.value); setFormError(""); }} autoComplete="new-password" aria-describedby="password-match-status" aria-invalid={confirmPassword.length > 0 && !passwordsMatch} required /><button type="button" className={auth.passwordToggle} onClick={() => setShowConfirmPassword((current) => !current)} aria-label={showConfirmPassword ? "Hide confirmation password" : "Show confirmation password"} aria-pressed={showConfirmPassword}>{showConfirmPassword ? "Hide" : "Show"}</button></div>
          </FormField>
          ...
          <Button type="submit" disabled={signingUp}>{signingUp ? "Creating account…" : `Continue as ${role}`}</Button>
        </form>
        <p className={auth.footer}>Already have an account? <Link className={auth.link} href="/login">Log in</Link></p>
      </section>
    </main>
  );
}
